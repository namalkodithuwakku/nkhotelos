...
export default function RoomsManager() {
  ...
  if (propertyLoading || loading) return <div className={ui.loading}>Loading room inventory…</div>;
    ...
    <div className={ui.page}>
      <div className={ui.toolbar}>
        <div>
          ...
          <p>{rooms.filter((room) => room.is_active).length} active rooms • {roomTypes.filter((type) => type.is_active).length} room types</p>
        </div>
              ...
              {rooms.length === 0 ? <div className={ui.empty}>No rooms configured.</div> : rooms.map((room) => {
                ...
                return (
                  <div className={ui.listRow} key={room.id}>
                    <div><strong>{room.room_name || `Room ${room.room_number}`}</strong><small>{room.room_number} • {type?.name || "Unassigned"}</small></div>
            ...
            <div className={ui.list}>
              {roomTypes.length === 0 ? <div className={ui.empty}>No room types configured.</div> : roomTypes.map((item) => (
                <div className={ui.listRow} key={item.id}>
                  <div><strong>{item.name}</strong><small>{item.code || "No code"} • {item.max_occupancy} guests</small></div>
                  <span className={ui.badge}>Min {property?.currency} {item.minimum_rate ?? "—"}</span>
                  <span className={ui.badge}>Std {property?.currency} {item.standard_rate ?? "—"}</span>
                  <span className={ui.badge}>Max {property?.currency} {item.maximum_rate ?? "—"}</span>
