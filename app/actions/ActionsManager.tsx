...
export default function ActionsManager() {
  ...
  if (propertyLoading || loading) return <div className={ui.loading}>Loading actions…</div>;
          ...
          <div className={ui.list}>
            {visible.length === 0 ? <div className={ui.empty}>No actions in this view.</div> : visible.map((item) => (
              <div className={ui.listRow} key={item.id}>
                <div><strong>{item.title}</strong><small>{item.module} • {item.due_date || "No due date"}</small></div>
